...
targetSpikeRankList <- lapply(c(110, 111, 112, 113, 114, 115), function(x) {
    c(x, 120)
})
########################################################################
#
...
for (tsprNum in seq(from = 1, to = 1)) {
    for (iterNum in seq(25)) {
        ...
        faustCountMatrix <- readRDS(file.path(
            normalizePath("."),
            jobPath,
            "faustData",
        ...
        names(faustDerivedClusterARIs) <- gsub(
            "~1~2~",
            "-",
            names(faustDerivedClusterARIs)
        )
        names(faustDerivedClusterARIs) <- gsub(
            "~2~2~",
            "+",
        ...
        names(faustDerivedClusterLabels) <- gsub(
            "~1~2~",
            "-",
            names(faustDerivedClusterLabels)
        )
        names(faustDerivedClusterLabels) <- gsub(
            "~2~2~",
            "+",
        ...
        faustResults$expectedFoldChange <- ((clusterProbVec[targetSpikeRanks][
            1
        ]) /
            (clusterProbVec[targetSpikeRanks][2]))
        ...
        for (cSampleName in names(sampleCountSummary)) {
            daPopTrueCounts[cSampleName] <- length(intersect(
                which(sampleLookups == cSampleName),
                daPopLookup
            ))
        }
        ...
        dsCitrusClusters <- dsCitrusResults$conditionRegressionResults$fileName$glmnet$differentialFeatures[[
            "cv.min"
        ]][["clusters"]]
        print(paste0("Citrus cluster length: ", length(dsCitrusClusters)))
        for (dsCitrusClusterID in dsCitrusClusters) {
            dsCitrusClusterLookup <- sort(unique(dsCitrusResults$citrus.foldClustering$allClustering$clusterMembership[[as.numeric(
                dsCitrusClusterID
            )]]))
            dsCitrusDataSubset <- dsCitrusData[
                dsCitrusClusterLookup,
                ,
                drop = FALSE
            ]
            dsCitrusSamplesInCluster <- sort(unique(dsCitrusDataSubset[,
                "fileId",
                drop = FALSE
            ]))
            dsCitrusClusterProps <- rep(0, 20)
            for (cSampNum in dsCitrusSamplesInCluster) {
                dsCitrusClusterProps[cSampNum] <- (length(which(
                    dsCitrusDataSubset[, "fileId"] == cSampNum
                )) /
                    subSampleSize)
            }
            dsCitrusPropDF <- cbind(dsCitrusPropDF, dsCitrusClusterProps)
            colnames(dsCitrusPropDF)[ncol(dsCitrusPropDF)] <- as.character(
                dsCitrusClusterID
            )
        }
        dsCitrusDerivedResults <- glmCVAUC(
            train = dsCitrusPropDF,
            y = "Responder",
            V = 5
        )
        #
        ...
        dsCitrusDerivedResults$expectedFoldChange <- ((clusterProbVec[
            targetSpikeRanks
        ][1]) /
            (clusterProbVec[targetSpikeRanks][2]))
        ...
        dsCitrusSimResults <- append(
            dsCitrusSimResults,
            list(dsCitrusDerivedResults)
        )
        saveRDS(
            dsCitrusSimResults,
            file.path(
        ...
        experiment_info$`group_id` <- as.character(
            as.numeric(modelResDF$Responder) - 1
        )
        ...
        compareDF <- modelResDF[, c(
            "subjectID",
            "childCount",
            "parentCount",
            "Responder"
        )]
        colnames(compareDF) <- c(
            "sampleID",
            "trueChildCount",
            "trueParentCount",
            "Responder"
        )
        diffCytModelDF <- inner_join(
            diffCytOutputDF,
            compareDF,
            by = c("sampleID")
        )
        diffCytModelDF$pct <- diffCytModelDF$childCount /
            diffCytModelDF$parentCount
        ...
        bestDiffcytClusterIndex[which(
            diffcytClustering == bestDiffcytCluster
        )] <- 1
        ...
        diffCytResults$bestClusterPhenotype <- diffcytDerivedLabels[
            bestDiffcytCluster
        ]
        diffCytResults$numberOfClusters <- numberOfClusters
        diffCytResults$expectedFoldChange <- ((clusterProbVec[targetSpikeRanks][
            1
        ]) /
            (clusterProbVec[targetSpikeRanks][2]))
        ...
        oracleFlowSOMResults$expectedFoldChange <- ((clusterProbVec[
            targetSpikeRanks
        ][1]) /
            (clusterProbVec[targetSpikeRanks][2]))
        ...
        oracleFlowsomSimResults <- append(
            oracleFlowsomSimResults,
            list(oracleFlowSOMResults)
        )
        saveRDS(
            oracleFlowsomSimResults,
            file.path(
        ...
        overpartFlowSOMResults$expectedFoldChange <- ((clusterProbVec[
            targetSpikeRanks
        ][1]) /
            (clusterProbVec[targetSpikeRanks][2]))
        ...
        opFlowSOMSimResults <- append(
            opFlowSOMSimResults,
            list(overpartFlowSOMResults)
        )
        saveRDS(
            opFlowSOMSimResults,
            file.path(
        ...
        kmeansResults$expectedFoldChange <- ((clusterProbVec[targetSpikeRanks][
            1
        ]) /
            (clusterProbVec[targetSpikeRanks][2]))
        ...
        phenographResults$expectedFoldChange <- ((clusterProbVec[
            targetSpikeRanks
        ][1]) /
            (clusterProbVec[targetSpikeRanks][2]))
        ...
        phenographSimResults <- append(
            phenographSimResults,
            list(phenographResults)
        )
        saveRDS(
            phenographSimResults,
            file.path(
        ...
        depecheResults$expectedFoldChange <- ((clusterProbVec[targetSpikeRanks][
            1
        ]) /
            (clusterProbVec[targetSpikeRanks][2]))
        ...
        PARCResults$expectedFoldChange <- ((clusterProbVec[targetSpikeRanks][
            1
        ]) /
            (clusterProbVec[targetSpikeRanks][2]))
        ...
        unlink(
            file.path(normalizePath("."), jobPath, "citrusData"),
            recursive = TRUE
        )
