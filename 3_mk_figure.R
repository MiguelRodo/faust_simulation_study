...
computeNCDF <- function(dataSource, methodName) {
    ...
    for (i in seq(length(lb))) {
        methodClusterAvg <- append(
            methodClusterAvg,
            median(dataSource[seq(lb[i], ub[i])])
        )
        methodClusterSD <- append(
            methodClusterSD,
            sd(dataSource[seq(lb[i], ub[i])])
        )
...
computeMetricDF <- function(dataSource, methodName) {
    ...
    currentTruthNum <- 5
    for (i in seq(length(lb))) {
        methodMetricMed <- append(
            methodMetricMed,
            median(dataSource[seq(lb[i], ub[i])])
        )
        methodMetricSD <- append(
            methodMetricSD,
            sd(dataSource[seq(lb[i], ub[i])])
        )
...
get_noc_plot <- function(simType) {
    faustNC <- readRDS(file.path(
        resultsDir,
        simType,
        "results",
        "final_faustNC.rds"
    ))
    FlowSOMNC <- readRDS(file.path(
        resultsDir,
        simType,
        "results",
        "final_FlowSOMNC.rds"
    ))
    FlowORCNC <- readRDS(file.path(
        resultsDir,
        simType,
        "results",
        "final_FlowORCNC.rds"
    ))
    flowMeansNC <- readRDS(file.path(
        resultsDir,
        simType,
        "results",
        "final_flowMeansNC.rds"
    ))
    depecheNC <- readRDS(file.path(
        resultsDir,
        simType,
        "results",
        "final_depecheNC.rds"
    ))
    phenographNC <- readRDS(file.path(
        resultsDir,
        simType,
        "results",
        "final_phenographNC.rds"
    ))
    kmeansNC <- readRDS(file.path(
        resultsDir,
        simType,
        "results",
        "final_kmeansNC.rds"
    ))
    parcNC <- readRDS(file.path(
        resultsDir,
        simType,
        "results",
        "final_parcNC.rds"
    ...
    p1 <- ggplot(
        plotDF,
        aes(x = truth, y = point.median, color = Method, fill = Method)
    ) +
        geom_point() +
        geom_line(position = position_dodge(width = 0.4)) +
    ...
    #
    FlowSOMMetric <- readRDS(file.path(
        resultsDir,
        simType,
        "results",
        "final_FlowSOMAll.rds"
    ))
    FlowORCMetric <- readRDS(file.path(
        resultsDir,
        simType,
        "results",
        "final_FlowORCAll.rds"
    ))
    flowMeansMetric <- readRDS(file.path(
        resultsDir,
        simType,
        "results",
        "final_flowMeansAll.rds"
    ))
    faustMetric <- readRDS(file.path(
        resultsDir,
        simType,
        "results",
        "final_faustAll.rds"
    ))
    faustMetricpheno <- readRDS(file.path(
        resultsDir,
        simType,
        "results",
        "final_faustPheno.rds"
    ))
    phenoMetric <- readRDS(file.path(
        resultsDir,
        simType,
        "results",
        "final_phenographAll.rds"
    ))
    depecheMetric <- readRDS(file.path(
        resultsDir,
        simType,
        "results",
        "final_depecheAll.rds"
    ))
    kmeansMetric <- readRDS(file.path(
        resultsDir,
        simType,
        "results",
        "final_kmeansAll.rds"
    ))
    parcMetric <- readRDS(file.path(
        resultsDir,
        simType,
        "results",
        "final_parcAll.rds"
    ...
    p2 <- ggplot(
        metricPlotDF,
        aes(x = truth, y = point.median, color = Method)
    ) +
        geom_hline(
            yintercept = 0.9,
            linetype = "dashed",
...
gatherResults <- function(methodType) {
    simResults <- readRDS(file.path(
        resPath,
        paste0(methodType, "_results.rds")
    ))
    resultsMatrix <- matrix(nrow = 0, ncol = 8)
    colnames(resultsMatrix) <- c(
        "tsprNum",
        "simIter",
        "expectedFoldChange",
        "cvAUC",
...
plotDF <- Reduce(
    rbind,
    lapply(gsub("_results.rds", "", methodsToCompare), gatherResults)
)
