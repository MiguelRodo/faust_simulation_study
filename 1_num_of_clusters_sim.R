...
for (numberOfClusters in seq(5, 125, by = 10)) {
    ...
    for (iterNum in seq(5)) {
        ...
        #
        clusterProbVec <- sort(
            refClusterProbVec[seq(numberOfClusters)],
            decreasing = TRUE
        )
        if (sum(clusterProbVec) < 1) {
            residualPV <- 1 - sum(clusterProbVec)
            clusterProbVec <- (clusterProbVec +
                (residualPV / length(clusterProbVec)))
        }
        if (sum(clusterProbVec) != 1) {
        ...
        for (sampleName in sampleNames(gs)) {
            ...
            #
            trueLabelVec[startL:endL] <- simmedExp[["labelsList"]][[which(
                names(simmedExp[["labelsList"]]) == sampleName
            )]]
            startL <- (endL + 1)
        }
        #
        ...
        flowMeansResult <- flowMeans(
            x = exprsMat,
            MaxN = (2 * numberOfClusters)
        )
        print("flowMeans complete")
        flowMeansClusterLabels <- as.numeric(flowMeansResult@Labels[[1]])
        flowMeansNC <- append(
            flowMeansNC,
            length(table(flowMeansClusterLabels))
        )
        ...
        FlowSOMResult <- FlowSOM(
            input = ffFlowSOM,
            colsToUse = seq(ncol(exprsMat)),
            maxMeta = 90
        )
        print("FlowSOM complete")
        FlowSOMClusterLabels <- as.numeric(GetMetaclusters(
            FlowSOMResult,
            FlowSOMResult$metaclustering
        ))
        ...
        phenographNC <- append(
            phenographNC,
            length(table(phenographClusterLabels))
        )
        ...
        depecheResult <- depeche(
            exprsMat,
            nCores = 10,
            k = (2 * numberOfClusters)
        )
        ...
        phenographScores <- ClusterR::external_validation(
            true_labels = trueNumericLabels,
        ...
        flowMeansScores <- ClusterR::external_validation(
            true_labels = trueNumericLabels,
        ...
        FlowSOMScores <- ClusterR::external_validation(
            true_labels = trueNumericLabels,
        ...
        FlowORCScores <- ClusterR::external_validation(
            true_labels = trueNumericLabels,
        ...
        depecheScores <- ClusterR::external_validation(
            true_labels = trueNumericLabels,
        ...
        kmeansScores <- ClusterR::external_validation(
            true_labels = trueNumericLabels,
        ...
        parcScores <- ClusterR::external_validation(
            true_labels = trueNumericLabels,
        ...
        #
        faustSamplePaths <- list.files(
            file.path(normalizePath("."), jobPath, "faustData", "sampleData"),
            full.names = TRUE
        )
        firstSample <- TRUE
        for (samplePath in faustSamplePaths) {
            fa <- read.table(
                paste0(samplePath, "/faustAnnotation.csv"),
                header = F,
                sep = "`"
            )[, 1]
            if (firstSample) {
        ...
        faustScoresAll <- ClusterR::external_validation(
            true_labels = trueNumericLabels,
        ...
        faustScoresPheno <- external_validation(
            true_labels = trueNumericLabels[faustPhenoLookup],
        ...
        phenographScoresPheno <- external_validation(
            true_labels = trueNumericLabels[faustPhenoLookup],
        ...
        depecheScoresPheno <- external_validation(
            true_labels = trueNumericLabels[faustPhenoLookup],
        ...
        FlowSOMScoresPheno <- external_validation(
            true_labels = trueNumericLabels[faustPhenoLookup],
        ...
        FlowORCScoresPheno <- external_validation(
            true_labels = trueNumericLabels[faustPhenoLookup],
        ...
        flowMeansScoresPheno <- external_validation(
            true_labels = trueNumericLabels[faustPhenoLookup],
        ...
        kmeansScoresPheno <- external_validation(
            true_labels = trueNumericLabels[faustPhenoLookup],
        ...
        parcScoresPheno <- external_validation(
            true_labels = trueNumericLabels[faustPhenoLookup],
        ...
        #
        unlink(
            file.path(normalizePath("."), jobPath, "faustData"),
            recursive = TRUE
        )
    }
    #
    ...
    saveRDS(
        ...
        file.path(
            ...
            jobPath,
            "results",
            "intermediate_phenographAll.rds"
        )
    )
    saveRDS(
        depecheAll,
    ...
    saveRDS(
        ...
        file.path(
            ...
            jobPath,
            "results",
            "intermediate_phenographPheno.rds"
        )
    )
    saveRDS(
        depechePheno,
    ...
    saveRDS(
        ...
        file.path(
            ...
            jobPath,
            "results",
            "intermediate_flowMeansPheno.rds"
        )
    )
    saveRDS(
        kmeansPheno,
...
saveRDS(
    faustPctAnn,
    file.path(normalizePath("."), jobPath, "results", "final_faustPctAnn.rds")
)

saveRDS(
    faustNC,
    file.path(normalizePath("."), jobPath, "results", "final_faustNC.rds")
)
saveRDS(
    FlowSOMNC,
    file.path(normalizePath("."), jobPath, "results", "final_FlowSOMNC.rds")
)
saveRDS(
    FlowORCNC,
    file.path(normalizePath("."), jobPath, "results", "final_FlowORCNC.rds")
)
saveRDS(
    phenographNC,
    file.path(normalizePath("."), jobPath, "results", "final_phenographNC.rds")
)
saveRDS(
    depecheNC,
    file.path(normalizePath("."), jobPath, "results", "final_depecheNC.rds")
)
saveRDS(
    flowMeansNC,
    file.path(normalizePath("."), jobPath, "results", "final_flowMeansNC.rds")
)
saveRDS(
    kmeansNC,
    file.path(normalizePath("."), jobPath, "results", "final_kmeansNC.rds")
)
saveRDS(
    parcNC,
    file.path(normalizePath("."), jobPath, "results", "final_parcNC.rds")
)

saveRDS(
    faustAll,
    file.path(normalizePath("."), jobPath, "results", "final_faustAll.rds")
)
saveRDS(
    FlowSOMAll,
    file.path(normalizePath("."), jobPath, "results", "final_FlowSOMAll.rds")
)
saveRDS(
    FlowORCAll,
    file.path(normalizePath("."), jobPath, "results", "final_FlowORCAll.rds")
)
saveRDS(
    phenographAll,
    file.path(normalizePath("."), jobPath, "results", "final_phenographAll.rds")
)
saveRDS(
    depecheAll,
    file.path(normalizePath("."), jobPath, "results", "final_depecheAll.rds")
)
saveRDS(
    flowMeansAll,
    file.path(normalizePath("."), jobPath, "results", "final_flowMeansAll.rds")
)
saveRDS(
    kmeansAll,
    file.path(normalizePath("."), jobPath, "results", "final_kmeansAll.rds")
)
saveRDS(
    parcAll,
    file.path(normalizePath("."), jobPath, "results", "final_parcAll.rds")
)

saveRDS(
    faustPheno,
    file.path(normalizePath("."), jobPath, "results", "final_faustPheno.rds")
)
saveRDS(
    FlowSOMPheno,
    file.path(normalizePath("."), jobPath, "results", "final_FlowSOMPheno.rds")
)
saveRDS(
    FlowORCPheno,
    file.path(normalizePath("."), jobPath, "results", "final_FlowORCPheno.rds")
)
saveRDS(
    phenographPheno,
    file.path(
        ...
        jobPath,
        "results",
        "final_phenographPheno.rds"
    )
)
saveRDS(
    depechePheno,
    file.path(normalizePath("."), jobPath, "results", "final_depechePheno.rds")
)
saveRDS(
    flowMeansPheno,
    file.path(
        ...
        jobPath,
        "results",
        "final_flowMeansPheno.rds"
    )
)
saveRDS(
    kmeansPheno,
    file.path(normalizePath("."), jobPath, "results", "final_kmeansPheno.rds")
)
saveRDS(
    parcPheno,
    file.path(normalizePath("."), jobPath, "results", "final_parcPheno.rds")
)
