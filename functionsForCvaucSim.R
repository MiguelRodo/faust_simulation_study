...
.cvAUCDoFit <- function(v, folds, train, y) {
    ...
    params <- list(
        x = train[-folds[[v]], -ycol, drop = FALSE],
...
glmCVAUC <- function(train, y = "V1", V = 10) {
    ...
    glmpredictions <- foreach(
        v = 1:V,
        .combine = "c",
        .packages = c("stats")
    ) %do%
        .cvAUCDoFit(v, folds, train, y)
...
computeAUCforBestCluster <- function(
    countMatrix,
    responderStatusDF,
    derivedARIs,
    derivedPhenotypes
...
getLabelsForClustering <- function(clusterSource, expressionSource) {
    ...
    for (mClusterID in methodClusterIDs) {
        mClusterExprs <- expressionSource[
            which(clusterSource == mClusterID),
            ,
            drop = FALSE
        ]
        ...
        mClusterLabel <- paste(
            paste0(paste0("V", seq(10)), labelInterpret),
            collapse = ""
        )
...
getCountMatrixFromClustering <- function(
    clusterSource,
    numberOfCols,
    sampleNames,
    sampleLookups
    ...
    for (sampleName in sampleNames) {
        ...
        for (clusterNum in seq(numberOfCols)) {
            ...
            if (clusterCount > 0) {
                colLookup <- which(
                    colnames(countMatrix) == as.character(clusterNum)
                )
